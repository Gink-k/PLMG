...
func (character *Character) Create() map[string]interface{} {
	...
	if character.ID <= 0 {
		return u.Message(false, "Failed to create character, connection error.")
	}

	response := u.Message(true, "Character has been created")
...
func (character *Character) Edit() map[string]interface{} {
	...
	response := u.Message(true, "Character has been edited")
	response["character"] = character
	return response
}

func (character *Character) Delete() map[string]interface{} {
	histories := GetAllHistBy(character.ID)
	for _, history := range histories {
		history.Delete()
	}
	GetDB().Delete(character)
	removePhoto(character.Photo)
	response := u.Message(true, "Character has been deleted")
	response["character"] = character
	return response
}
...
func GetCharacter(id uint) *Character {
	char := &Character{}
