    ...
    function handleNavHeight(event) {
        if (event.target.closest("a") || event.type == "load") {
            setTimeout(() => {
                ...
                nav.style.height = profilePanel ? profilePanel.scrollHeight + "px" : ""
            }, 50)
        }
    }
...
function ItemPanel(props) {
    ...
    React.useEffect(() => {
        ...
        elements = items.map((value, index) => {
            return <ItemRow id={idStart + index} key={idStart + index} item={value} itemName={itemName} pathName={props.pathName} handleClick={handleClick}/>
        })
...
function ItemView(props) {
    ...
    const detaildsBtn = compObj.next ? <a onClick={handleDetailsClick}>Детали</a> : ""
    ...
    return (
        <Wrap className={className}>
            {Object.entries(item).map(([key, value]) => {
                ...
                if (key == "ID") return
                return (
                    <Wrap className={itemClass} key={key}>
                        {title}
                        {elem}
                    </Wrap>
                )
    ...
    return (
        <Wrap className="acc-control-panel">
            {props.values.map((value, index)=>{
                return (
                    <Wrap className="control-btn" key={value} onMouseDown={(e)=>e.preventDefault()}>
                        <a className="item-control-button" onClick={props.onClick[index]}>{value}</a>
                    </Wrap>
                )
            })}
            {props.children}
